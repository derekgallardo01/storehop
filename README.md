...
walkthrough at
[`docs/ios-app-store-submission.md`](docs/ios-app-store-submission.md).
...
- v0.5.15 iOS catches up to Android: shared UndoBar component on
         iOS ItemsListView + ManageCategoriesView (× close, swipe,
...
- v0.6.10 iOS parity catch-up (iOS-only; Android unchanged). Three
          long-standing gaps closed: the StorePicker banner now has
          the same "Most at <store> (<N>)" routing hint + per-store
          breakdown Android shipped in v0.5.6; Items / Shop-at-Store
          / Store Picker now have empty-state illustrations (SF
          Symbol + title + body) matching Android v0.6.8; the
          OnSurfaceVariant + Secondary palette colours were darkened
          to fix a WCAG AA contrast failure (4.20:1 → 5.30:1) the
          Android theme audit caught in v0.6.8.
...
- v0.6.6 Follow-up fix: drag now actually commits on Manage
         ...
         spot. Root cause was a `remember(categories)` key on the
         ...
         EditAisleOrder pattern.
- v0.6.5 Fix Mike-reported gesture conflict on Manage Categories:
         trying to drag a row was triggering bulk-select instead.
         The v0.6.4 long-press-to-enter-selection handler covered
         the whole Card, so the drag-handle's own long-press race
         was losing. Scoped the selection gesture to just the
         checkbox + text region; the drag handle and overflow-menu
         button now live outside that tap target.
- v0.6.4 Manage Categories: drag-to-reorder + bulk select to
         delete + multi-add. Schema migration v6 → v7 adds a
         displayOrder column to categories; the backfill
         dense-ranks by name so the existing list order is
         preserved on first open. iOS uses the platform-idiomatic
         EditButton for selection + drag. 424 → 440 unit tests.
- v0.6.3 Fix Mike-reported regression: the in-app update sheet
         shows twice on tap. Root cause was a race window in
         the v0.5.7 guard where Play's bottom-sheet dismiss
         happens before installStatus transitions out of
         UNKNOWN, so the next onResume re-prompted. Added a
         per-activity session flag in AppUpdateController.
