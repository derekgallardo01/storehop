...
final class MockFirebaseAuthClient: FirebaseAuthClient, @unchecked Sendable {
    ...
    func makeAppleCredential(idToken: String, rawNonce: String) -> AnyAuthCredential {
        AnyAuthCredential(payload: ("apple", idToken, rawNonce))
    }

    @discardableResult
    func linkAnonymousWithApple(credential: AnyAuthCredential) async throws -> String {
        // Same defaulting story as the Google link path — tests that
        // exercise SIWA directly will route through SignInWithAppleUseCase
        // rather than driving this mock by hand.
        lock.lock(); let current = uid ?? "linked"; uid = current; anonymous = false; lock.unlock()
        return current
    }
    ...
    func signInWithApple(credential: AnyAuthCredential) async throws -> String {
        lock.lock(); uid = "apple_uid"; anonymous = false; lock.unlock()
        return "apple_uid"
    }
}
