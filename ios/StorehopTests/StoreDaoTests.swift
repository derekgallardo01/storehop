...
final class StoreDaoTests: XCTestCase {
    ...
    func testNextDisplayOrderReturnsZeroOnEmptyTable() async throws {
        let (_, dao) = try setup()
        let order = try await dao.nextDisplayOrder(householdId: "u1")
    ...
    func testFindAnyByNameSurfacesTombstonedRowsForResurrection() async throws {
        ...
        try await dao.softDelete(householdId: "u1", id: "s1", now: 1_000)

        let live = try await dao.findByName(householdId: "u1", name: "Lidl")
        XCTAssertNil(live, "findByName must filter out tombstones")
        let any = try await dao.findAnyByName(householdId: "u1", name: "Lidl")
    ...
    func testRestoreFromTombstoneClearsDeletedAtAndFlagsPendingSync() async throws {
        ...
        try await dao.softDelete(householdId: "u1", id: "s1", now: 1_000)
        try await dao.restoreFromTombstone(householdId: "u1", id: "s1", now: 2_000)

        let store = try await dao.findById(householdId: "u1", id: "s1")
    ...
    func testObserveAllExcludesArchivedByDefault() async throws {
        ...
        let visible = try await db.queue.read { conn in
            try Store.fetchAll(conn, sql: """
                SELECT * FROM stores
                WHERE householdId = ? AND deletedAt IS NULL AND (? = 1 OR isArchived = 0)
