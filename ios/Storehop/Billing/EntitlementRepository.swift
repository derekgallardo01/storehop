...
///      — `Transaction.currentEntitlements` says the user owns either
///      `premium_lifetime_v2` (build 53+ live product) OR the legacy
///      `premium_lifetime` (pre-v2 transactions still on Apple's side)
///      and the transaction isn't revoked. See
///      `StoreKitManager.entitlementGrantingProductIds` for the
