...
enum Migrations {
    static func migrator() -> DatabaseMigrator {
        ...
        // v9 (v0.9.0): one-off stores. Mirrors Android's MIGRATION_9_10.
        // Adds `isOneOff INTEGER NOT NULL DEFAULT 0` to the stores table
        // plus a composite index on (householdId, isOneOff) that backs
        // the master-items-list EXISTS subquery introduced in
        // `ItemDao.observeAll`.
        //
        // No data backfill needed — every existing store is "regular"
        // (isOneOff = false), which is the column default. Users opt
        // stores into one-off mode explicitly through the Add Store
        // dialog or the per-store toggle.
        //
        // Note: Android numbers this migration v10 because it has an
        // intermediate v8 -> v9 step that creates an `alive_item_store_xref`
        // SQL view to filter tombstoned xrefs in `@Junction` joins. iOS
        // doesn't need that view (raw SQL on this side already filters
        // `deletedAt IS NULL` at every join site), so we keep our local
        // sequence at v8 -> v9. The resulting `stores` table schema is
        // identical to Android's v10.
        migrator.registerMigration("v9_stores_one_off") { db in
            try db.execute(sql: """
