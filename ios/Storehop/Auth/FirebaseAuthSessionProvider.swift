        ...
        do {
            ...
            if cloudHasData {
                ...
                case let .failure(reason):
                    NSLog("[Storehop] pull failed (uid=\(uid), hid=\(householdId)): \(reason)")
                    await pullStateRepo.set(.failed, for: uid, reason: reason)
                }
            } else {
            ...
            // to the cloud. Capture the real error so device logs +
            // the Settings banner can surface it (otherwise users hit a
            // generic banner with no clue what to do).
            let reason = "peek failed: \(error.localizedDescription)"
            NSLog("[Storehop] sync error (uid=\(uid), hid=\(householdId)): \(reason) | full: \(error)")
            await pullStateRepo.set(.failed, for: uid, reason: reason)
        }
    }
