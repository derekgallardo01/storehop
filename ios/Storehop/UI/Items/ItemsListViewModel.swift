...
final class ItemsListViewModel {
    ...
    /// v0.8.1: live store list for the bulk-tag picker dialog. Populated by
    /// the same archived-excluded observation `ItemFormViewModel` uses.
    var stores: [Store] = []
    /// v0.8.1: item IDs the user has multi-selected (long-press → tap to
    /// extend). Non-empty set means the screen is in *selection mode*: the
    /// toolbar swaps to a contextual variant with a "[N] selected" title +
    /// a "Tag to stores…" action, row taps toggle membership instead of
    /// opening the editor, and the +/- toggle is hidden.
    var selectedItemIds: Set<String> = []
    /// v0.8.1: convenience over `selectedItemIds.isEmpty == false`. The
    /// view reads this to flip toolbar / FAB / row behavior.
    var isInSelectionMode: Bool { !selectedItemIds.isEmpty }
    var query: String = "" {
        didSet { refresh() }
    }
    ...
    func bind() {
        ...
        // v0.8.1: live store list for the bulk-tag picker dialog. Mirrors
        // the archived-excluded observation `ItemFormViewModel.bind` uses.
        storesBinder.bind(
            session: session,
            emptyValue: [Store]()
    ...
    // MARK: - v0.8.1 bulk-tag selection mode

    /// Toggle `itemId` in the selection set. First selected item enters
    /// selection mode; removing the last one exits it. Used by both the
    ...
    /// contextual toolbar's close action.
    func clearSelection() {
        selectedItemIds = []
    }
    ...
    /// either set is empty.
    func applyBulkStores(_ storeIdsToAdd: Set<String>) {
        let ids = selectedItemIds
        guard !ids.isEmpty && !storeIdsToAdd.isEmpty else { return }
        Task { [weak self, itemRepository] in
            do {
                try await itemRepository.bulkTagStoresForItems(
                    itemIds: ids,
                    storeIdsToAdd: storeIdsToAdd
                )
                await MainActor.run { self?.selectedItemIds = [] }
            } catch {
                // Sync writes are best-effort; the live observation will
                // re-render with whatever the DB actually contains. Leave
