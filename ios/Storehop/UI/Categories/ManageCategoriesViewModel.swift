...
final class ManageCategoriesViewModel {
    ...
    func renameCategory(id: String, name: String) async -> String? {
        ...
        guard !trimmed.isEmpty else { return L("error_category_name_empty") }
        do {
            try await categoryRepository.rename(id: id, name: trimmed)
            return nil
        } catch {
            return L("error_could_not_rename_category")
        }
    }
    ...
    func addManyCategories(raw: String) async -> BulkAddResult {
        ...
        if unique.isEmpty {
            return BulkAddResult(
                added: 0,
                duplicates: 0,
                errors: [L("error_category_name_empty")],
            )
        }
        var added = 0
        var duplicates = 0
        ...
        for name in unique {
            do {
                ...
                errors.append("\(L("error_could_not_add_category")): \(name)")
            }
        }
