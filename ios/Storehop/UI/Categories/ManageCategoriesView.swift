...
struct ManageCategoriesView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Rebind on every appear: NavigationStack pushes cancel the
            // GRDB observation via onDisappear → teardown(); bind() is
            // idempotent (cancels prior subscriptions before re-subscribing).
            viewModel?.bind()
        }
    ...
    private func content(viewModel: ManageCategoriesViewModel) -> some View {
        ...
        return ZStack(alignment: .bottom) {
            ...
            .navigationTitle(L("title_manage_categories"))
                ...
                if editMode?.wrappedValue.isEditing == true && !vm.selectedIds.isEmpty {
                    ToolbarItem(placement: .bottomBar) {
                        Button(role: .destructive) {
                            ...
                            Label(
                                String(format: L("manage_categories_selected_count %lld"), vm.selectedIds.count) +
                                    " · " + L("action_delete"),
                                systemImage: "trash"
                            )
                        }
            ...
            if let undoName = viewModel.pendingUndoName {
                UndoBar(
                    message: String(format: L("undo_category_deleted %@"), undoName),
            ...
            if viewModel.pendingBulkUndoDeletedAt != nil {
                UndoBar(
                    message: String(
                        format: L("undo_categories_deleted_count %lld"),
        ...
        .sheet(item: $pendingRename) { category in
            CategoryNameDialog(
                title: L("rename_category_dialog_title"),
        ...
        .alert(item: $pendingDelete) { category in
            Alert(
                title: Text(String(format: L("delete_category_dialog_title %@"), category.name)),
                message: Text(L("delete_category_dialog_message")),
        ...
        .alert(
            String(
                format: L("delete_categories_dialog_title %lld"),
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if category.isSeeded {
                    Text(L("category_seeded_subtitle"))
...
private struct BulkAddCategoryDialog: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    Text(L("add_category_multi_help"))
                        .font(StorehopTypography.bodySmall)
                        .foregroundStyle(StorehopColors.onSurfaceVariant)
                ...
                if let summary {
                    Section {
                        Text(
                            String(
                                format: L("add_category_summary %1$lld %2$lld"),
            ...
            .navigationTitle(L("add_category_dialog_title"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(L("action_cancel"), action: onDismiss).disabled(saving)
                }
