...
struct ForceSyncSection: View {
    ...
    var body: some View {
        Section {
            VStack(alignment: .leading, spacing: 8) {
                Text(String(localized: "settings_force_sync_explainer"))
                ...
                case .syncing(let n):
                    HStack(spacing: 12) {
                        ProgressView()
                        Text(String(format: String(localized: "settings_force_sync_in_progress %lld"), n))
                            .font(StorehopTypography.bodyMedium)
                    }

                case .safeToUninstall:
                    Text(String(localized: "settings_force_sync_safe"))
                        .font(StorehopTypography.bodyMedium)
                        .foregroundStyle(.green)
                    Button(action: { state = .idle }) {
                        Text(String(localized: "settings_force_sync_button_done"))
                            .frame(maxWidth: .infinity)
                    }
                    .buttonStyle(.bordered)

                case .failed(let n):
                    Text(String(format: String(localized: "settings_force_sync_failed %lld"), n))
                        .font(StorehopTypography.bodyMedium)
                        .foregroundStyle(.red)
                    Button(action: forceSyncTapped) {
                        Text(String(localized: "settings_force_sync_button_retry"))
                            .frame(maxWidth: .infinity)
                    }
                    .buttonStyle(.borderedProminent)
                }
            ...
            Text(String(localized: "settings_force_sync_title"))
        }
        .task {
    ...
    private var idleLabel: String {
        if pendingCount > 0 {
            String(format: String(localized: "settings_force_sync_button_pending %lld"), pendingCount)
        } else {
            String(localized: "settings_force_sync_button_idle")
        }
    }
