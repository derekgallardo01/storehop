...
struct DataSettingsSection: View {
    ...
    /// v0.8.1.2: when an un-entitled user taps Export, show the
    /// `PremiumUpgradeSheet` so they see what Premium unlocks before
    /// Apple's bare StoreKit dialog opens. Previously the locked-state
    /// tap dropped users straight into the purchase chrome with no
    /// explanation.
    ...
    var body: some View {
        Section {
            ...
            // App-Store-localized price; tapping opens the upgrade sheet
            // (which then launches the purchase). Import stays
            // unconditionally free — onboarding hook for users migrating
            // from another app.
            actionRow(
            ...
            actionRow(
                title: L("action_import_items"),
                systemImage: "square.and.arrow.down"
            ) { importMode = .items }
            actionRow(
                title: L("action_import_categories"),
                systemImage: "square.and.arrow.down"
            ...
            Text(L("settings_data_section_title"))
        } footer: {
            Text(L("settings_data_section_subtitle"))
        }
        .fileExporter(
            isPresented: Binding(
    ...
    private var exportItemsLabel: String {
        if entitlement.isUnlocked { return L("action_export_items") }
        if let price = localizedPrice {
            return String(format: L("premium_locked_export_label %@"), price)
        }
        return L("action_export_items")
    }

    private var exportCategoriesLabel: String {
        if entitlement.isUnlocked { return L("action_export_categories") }
        if let price = localizedPrice {
            return String(format: L("premium_locked_export_categories_label %@"), price)
        }
        return L("action_export_categories")
    }
    ...
    private func importResultMessage(_ result: ImportResult) -> String {
        String(
            format: L("import_result_summary %lld %lld %lld %lld"),
