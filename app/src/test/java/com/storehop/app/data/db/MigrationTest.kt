...
class MigrationTest {
    ...
    @Test fun `v9 to v10 adds isOneOff column to stores with default 0 and a composite index`() {
        // v0.9: marks a store as a "one-off" store for non-recurring
        // purchases. Migration is column + index only; no data
        // backfill (every existing store stays regular).
        withV1Db { db ->
            db.execSQL("INSERT INTO stores(id,name,colorArgb,isArchived,isSeeded,userId,createdAt,updatedAt,deletedAt) " +
                "VALUES('s1','Lidl',NULL,0,0,'u',1,1,NULL)")
        }.migrateTo(10).use { db ->
            ...
            db.query("PRAGMA table_info(`stores`)").use { c ->
            ...
            assertThat(cols).containsKey("isOneOff")
            assertThat(cols.getValue("isOneOff").first).isEqualTo("INTEGER")
            assertThat(cols.getValue("isOneOff").second).isEqualTo("0")
            // Existing row got the default.
            db.queryRow("SELECT isOneOff FROM stores WHERE id='s1'") { c ->
                assertThat(c.getInt(0)).isEqualTo(0)
            }
            // Composite index exists for the master-list EXISTS subquery.
            val indexNames = mutableListOf<String>()
            db.query("PRAGMA index_list(`stores`)").use { c ->
                val nameIdx = c.getColumnIndexOrThrow("name")
                while (c.moveToNext()) indexNames += c.getString(nameIdx)
            }
            assertThat(indexNames).contains("index_stores_householdId_isOneOff")
        }
    }
