...
class ItemFormViewModelTest {
    ...
    // ---- v0.9 one-off store: hide staple/priority toggles ---------------

    @Test fun `allPickedStoresAreOneOff is false when no stores are picked`() = runTest {
        every { storeRepo.observeAll(any()) } returns flowOf(
            listOf(store("s_lidl", isOneOff = false)),
        )
        val vm = newAddVm()
        vm.allPickedStoresAreOneOff.test {
    ...
    @Test fun `allPickedStoresAreOneOff is true when every picked store is one-off`() = runTest {
        every { storeRepo.observeAll(any()) } returns flowOf(
            listOf(
                store("s_online_oneoff", isOneOff = true),
                store("s_hardware_oneoff", isOneOff = true),
                store("s_lidl", isOneOff = false),
            ),
        )
        val vm = newAddVm()
        // Subscribe to keep the WhileSubscribed combine alive, then read
        // the StateFlow's .value after toggles + advanceUntilIdle.
        vm.allPickedStoresAreOneOff.test {
            awaitItem()  // initial false (empty selection)
            vm.toggleStore("s_online_oneoff")
            vm.toggleStore("s_hardware_oneoff")
            advanceUntilIdle()
            assertThat(vm.allPickedStoresAreOneOff.value).isTrue()
            cancelAndIgnoreRemainingEvents()
        }
    }

    @Test fun `allPickedStoresAreOneOff is false when at least one picked store is regular`() = runTest {
        every { storeRepo.observeAll(any()) } returns flowOf(
            listOf(
                store("s_online_oneoff", isOneOff = true),
                store("s_lidl", isOneOff = false),
        ...
        vm.allPickedStoresAreOneOff.test {
            awaitItem()
            vm.toggleStore("s_online_oneoff")
            vm.toggleStore("s_lidl")
            advanceUntilIdle()
            assertThat(vm.allPickedStoresAreOneOff.value).isFalse()
            cancelAndIgnoreRemainingEvents()
        }
    }

    private fun store(id: String, isOneOff: Boolean): com.storehop.app.data.entity.Store =
