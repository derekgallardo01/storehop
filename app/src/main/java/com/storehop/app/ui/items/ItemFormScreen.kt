...
fun ItemFormScreen(
    ...
    val allPickedStoresAreOneOff by viewModel.allPickedStoresAreOneOff.collectAsState()
    ...
    Scaffold(
        ...
        Column(
            ...
            // v0.9: hide staple + priority toggles when every picked
            // store is one-off. "Always recurring" and "buy once" don't
            // combine, so showing them would be confusing. The divider
            // above is still useful so the remaining sections still get
            // visual separation; we only skip the toggle rows.
            if (!allPickedStoresAreOneOff) {
                ToggleRow(
