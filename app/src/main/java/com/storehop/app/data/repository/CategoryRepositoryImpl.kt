 ...
 * DAOs (itemDao, scoDao) still take `userId: String` named parameters but
 * receive `householdId` as the value -- identical in single-member
 * households; correct semantics once those DAOs flip their SQL filter.
 */
class CategoryRepositoryImpl @Inject constructor(
    ...
    override suspend fun addCategory(name: String, icon: String?): String = db.withTransaction {
        ...
        when {
            existing == null -> {
                ...
                val nextOrder = (dao.maxDisplayOrder(householdId) ?: -1) + 1
                dao.upsert(
                    Category(
                        id = id,
                        name = trimmed,
    ...
    override suspend fun softDelete(id: String) = db.withTransaction {
        ...
        itemDao.clearCategoryReferences(householdId, id, now)
        scoDao.softDeleteForCategory(householdId, id, now)
    }
    ...
    override suspend fun undoSoftDelete(id: String) = db.withTransaction {
        ...
        val category = dao.findAnyById(householdId, id) ?: return@withTransaction
        val deletedAt = category.deletedAt ?: return@withTransaction
        val now = clock.millis()
        dao.restoreFromTombstone(householdId, id, now)
        scoDao.restoreCascadeForCategory(householdId, id, deletedAt, now)
        ...
        itemDao.restoreCategoryReferences(householdId, id, deletedAt, now)
    }
    ...
    override suspend fun reorder(orderedIds: List<String>) = db.withTransaction {
        ...
        orderedIds.forEachIndexed { index, id ->
            dao.updateDisplayOrder(householdId, id, index, now)
        }
    }
    ...
    override suspend fun undoSoftDeleteMany(deletedAt: Long) = db.withTransaction {
        ...
        val tombstoned = dao.findTombstonedAt(householdId, deletedAt)
        tombstoned.forEach { category ->
            dao.restoreFromTombstone(householdId, category.id, now)
            scoDao.restoreCascadeForCategory(householdId, category.id, deletedAt, now)
            itemDao.restoreCategoryReferences(householdId, category.id, deletedAt, now)
        }
    }
