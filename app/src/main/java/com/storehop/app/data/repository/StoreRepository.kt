...
interface StoreRepository {
    ...
    suspend fun addStore(name: String, colorArgb: Int? = null, isOneOff: Boolean = false): String
    suspend fun rename(id: String, name: String)
    suspend fun setColor(id: String, colorArgb: Int?)
    suspend fun setArchived(id: String, archived: Boolean)

    /**
     * v0.9: flip a store's one-off flag. One-off stores hold non-recurring
     * purchases (e.g. "Online (One Off)" for a new couch). Items whose
     * alive xrefs all point at one-off stores are hidden from the master
     * Items list (see `ItemDao.observeAll`). Cross-store cascade still
     * applies for mixed-tag items. The critical-needs banner and picker
     * badge counts skip one-off stores by default.
     */
    suspend fun setOneOff(id: String, isOneOff: Boolean)
