...
interface ItemDao {
    ...
    /**
     * Master Items list source. Returns items in the active household
     * with the following inclusion rule:
     *  - SHOW if the item has zero alive xrefs (untagged items, e.g.
     *    fresh CSV imports waiting to be tagged via the form).
     *  - SHOW if at least one alive xref points at a regular (non-one-
     *    off) store. Items that span regular + one-off stores still
     *    appear on the master list as usual.
     *  - HIDE if the item has alive xrefs and ALL of them point at
     *    one-off stores. These items only surface inside their one-off
     *    store's Shop view via [ShoppingDao.shoppingListForStore].
     *
     * The `alive_item_store_xref` view (v0.8.1) filters tombstoned
     * bridge rows so the subquery stays correct under soft-deletes.
     * The composite index `index_stores_householdId_isOneOff` (v0.9)
     * backs the store lookup.
     */
    @Transaction
    @Query(
        """
        ...
        WHERE householdId = :householdId
          ...
          AND (
              NOT EXISTS (
                  SELECT 1 FROM alive_item_store_xref aisx
                  WHERE aisx.itemId = items.id
              )
              OR EXISTS (
                  SELECT 1 FROM alive_item_store_xref aisx
                  JOIN stores s ON s.id = aisx.storeId
                  WHERE aisx.itemId = items.id
                    AND s.isOneOff = 0
